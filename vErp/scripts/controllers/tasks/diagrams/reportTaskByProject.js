angular.module('xMoney')
	.directive('reportTaskByProject', function (xService, ngDialog, $timeout, SETTINGS) {
		return {
			restrict: 'E',
			scope: {},
					...
					var loadChart = function (loadParams) {
						...
						xService.requestServerNative(loadParams, function (err, resp) {
							...
							if (!resp.TaskReportDS.TaskSummary) {
								...
								$scope.chartByProjectConfig.options.chart.type = (chartData['Category'].length) > 4 ? 'bar' : 'column';
								$scope.chartByProjectConfig.options.chart.height = (chartData['Category'].length) > 4 ? chartData['Category'].length * 115 : 400;
			...
			templateUrl: 'views/tasks/diagrams/reportTaskByProject.html'
		}
	})
;