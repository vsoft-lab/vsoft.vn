'use strict';
//8.4.5 Giá trị hợp đồng và các đợt thanh toán
var myApp = angular.module('xMoney');
myApp.directive('reportContractPaymentStatus', function (xService, ngDialog) {
    return {
        restrict: 'E',
        scope: true,
            ...
            $scope.refreshChart = function (params) {
                ...
                xService.requestServerNative(params, function (err, data) {
                    ...
                    if (result != null) {
                        ...
                        var sumPayment = { name: "Đã thanh toán", data: [] }; // đã thanh toán
                        var sumPayment1 = { name: "Đợt 1", data: [] };
                        var sumPayment2 = { name: "Đợt 2", data: [] };
                        var sumPayment3 = { name: "Đợt 3", data: [] };
                        var sumUnpaid = { name: "Chưa thanh toán", data: [] };
                        ...
                        for (var i = 0; i < result.length; i++) {
                            if (result[i].OfficialNumber != null && result[i].OfficialNumber != undefined) {
                                ...
                                sumPayment.data.push(convetToFloat(result[i].Payment1) + convetToFloat(result[i].Payment2) + convetToFloat(result[i].Payment3));
                                sumPayment1.data.push(convetToFloat(result[i].Payment1));
                                sumPayment2.data.push(convetToFloat(result[i].Payment2));
                                sumPayment3.data.push(convetToFloat(result[i].Payment3));
                                sumUnpaid.data.push(convetToFloat(result[i].TotalValue) - (convetToFloat(result[i].Payment1) + convetToFloat(result[i].Payment2) + convetToFloat(result[i].Payment3)));
                            }
                        }

                        $scope.chartSeries = [sumValue, sumPayment, sumPayment1, sumPayment2, sumPayment3, sumUnpaid];
                        ...
                        if ($scope.chartCategories.length > 0) {
                            ...
                            $scope.chartConfig = {
                                options: {
                                    ...
                                    title: { text: 'Giá trị hợp đồng và các đợt thanh toán' },
        ...
        templateUrl: 'views/contracts/diagrams/report-contract-payment-status.html'
    };
});
